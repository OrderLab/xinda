from trial_setup_context import get_trial_setup_context_from_path
from tools import read_raw_logfile
import re
import pandas as pd
...
def _runtime_parser_cassandra(log_raw):
    ...
    data = matches[:-2]  # get rid of the dirty tail
    df = pd.DataFrame(data, columns=["time(sec)", "throughput(ops/sec)"])
    return df
...
def _runtime_parser_crdb(log_raw):
    ...
    df = pd.DataFrame(data, columns=["time(sec)", "throughput(ops/sec)", "errors"])
    return df
...
def _runtime_parser_hadoop():
    pass


def _runtime_parser_hbase():
    pass
