...
for iter in ${iteration_2nd_half[@]}; do
    ...
    echo "## [$(date +%s%N), $(date +"%H:%M:%S")] Sourcing ${iter}/2nd-half $location restart-${severity}-dur${duration}-${start_time}-${end_time}.sh now" >> $meta_log_loc
    bash $main_bash_loc $wkl 10000 10000000 slow $location restart-${severity}-dur${duration}-${start_time}-${end_time} $severity $iter
    echo "## [$(date +%s%N), $(date +"%H:%M:%S")] Sourcing ${iter}/2nd-half $location norestart-${severity}-dur${duration}-${start_time}-${end_time}.sh now" >> $meta_log_loc
...
cd $data_dir
log_dir1=r10000_o10000000
log_dir2=${log_dir1}/wklb_logs
create_dir_if_not_exist $log_dir1
create_dir_if_not_exist $log_dir2
meta_log_loc=${data_dir}/${log_dir2}/wklb_meta.log
