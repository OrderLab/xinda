...
echo "## [$(date +%s%N), $(date +"%H:%M:%S"), $cur_time] Sleep for $(echo "39-$cur_time" | bc ) till next command" >> $rlog_pos
echo "39-$cur_time" | bc | xargs sleep
