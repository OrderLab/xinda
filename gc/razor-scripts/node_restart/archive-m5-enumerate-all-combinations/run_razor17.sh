...
for duration in ${duration_ary[@]}; do
    for start_time in ${start_time_ary[@]}; do
        ...
        echo "## [$(date +%s%N), $(date +"%H:%M:%S")] Sourcing $2 restart-slow3-dur${duration}-${start_time}-${end_time}.sh now" >> $meta_log_loc
        bash $main_bash_loc $1 10000 10000000 slow $2 restart-slow3-dur${duration}-${start_time}-${end_time} slow3
        echo "## [$(date +%s%N), $(date +"%H:%M:%S")] Sourcing $2 norestart-slow3-dur${duration}-${start_time}-${end_time}.sh now" >> $meta_log_loc
        bash $main_bash_loc $1 10000 10000000 slow $2 norestart-slow3-dur${duration}-${start_time}-${end_time} slow3
    done
done
