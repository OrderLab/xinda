...
echo "## [$(date +%s%N), $(date +"%H:%M:%S"), $cur_time] Sleep for $(echo "19-$cur_time" | bc ) till next command" >> $rlog_pos
echo "19-$cur_time" | bc | xargs sleep
...
cur_time=$(($(date +%s%N) - begin_time))
cur_time=$(echo "$cur_time / 10^9" | bc -l | xargs printf "%.3f")
if (( $(echo "$cur_time < 119" | bc -l) )); then
	blockade --config $blockade_file slow $5 2000ms 200ms distribution normal
	echo "## [$(date +%s%N), $(date +"%H:%M:%S")] fault resumed" >> $rlog_pos
	########## fault resumed ########
	cur_time=$(($(date +%s%N) - begin_time))
	cur_time=$(echo "$cur_time / 10^9" | bc -l | xargs printf "%.3f")
	echo "## [$(date +%s%N), $(date +"%H:%M:%S"), $cur_time] Sleep for $(echo "119-$cur_time" | bc ) till the end" >> $rlog_pos
	echo "119-$cur_time" | bc | xargs sleep
...
echo "## 19	119	100	fault" >> $rlog_pos
