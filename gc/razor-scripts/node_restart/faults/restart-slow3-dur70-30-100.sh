...
echo "## [$(date +%s%N), $(date +"%H:%M:%S"), $cur_time] Sleep for $(echo "30-$cur_time" | bc ) till next command" >> $rlog_pos
echo "30-$cur_time" | bc | xargs sleep
...
cur_time=$(($(date +%s%N) - begin_time))
cur_time=$(echo "$cur_time / 10^9" | bc -l | xargs printf "%.3f")
if (( $(echo "$cur_time < 100" | bc -l) )); then
	blockade --config $blockade_file slow $5 500ms 100ms distribution normal
	echo "## [$(date +%s%N), $(date +"%H:%M:%S")] fault resumed" >> $rlog_pos
	########## fault resumed ########
	cur_time=$(($(date +%s%N) - begin_time))
	cur_time=$(echo "$cur_time / 10^9" | bc -l | xargs printf "%.3f")
	echo "## [$(date +%s%N), $(date +"%H:%M:%S"), $cur_time] Sleep for $(echo "100-$cur_time" | bc ) till the end" >> $rlog_pos
	echo "100-$cur_time" | bc | xargs sleep
