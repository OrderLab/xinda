...
## MapReduce + HDFS
## Currently at least available on razor19
## $1 a/b/c/d/e/f 
## $2 recordcount=10000 
## $3 operationcount 
...
cd /data/ruiming/xinda/xinda-software/hadoop-3.2.1/hadoop-mapreduce-project/hadoop-mapreduce-client/hadoop-mapreduce-client-jobclient/target
...
function running_mrbench_iteratively() {
    ...
    for iter in ${ary[@]}; do
        ...
        while ! cat ${data_dir}/${log_dir2}/runtime-$2-mrbench${iter}.log  | grep -q "END-OF-MRBench" ; do
            ...
            docker exec datanode2 yarn jar $mrbench_dir mrbench -reduces 3 >> ${data_dir}/${log_dir2}/raw-$2-mrbench.log 2>> ${data_dir}/${log_dir2}/runtime-$2-mrbench${iter}.log 
